...
As seen in table  \@ref(tab:SummaryPlantPA) there are `r readRDS("FS.rds") %>% 
    ...
    dplyr::filter(Max_VIF <= 10) %>% 
    mutate(DeltaAICc = AICc - min(AICc)) %>% 
    relocate(DeltaAICc, .after = AICc) %>% 
    dplyr::filter(DeltaAICc <= 2) %>% 
    nrow()` models within 2 AICc where the max VIF is lower than 5 of each other, you can see there how many times a variable has been selected
