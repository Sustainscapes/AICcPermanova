...
readRDS("FS.rds") %>% 
    ...
    dplyr::select(-Dataset, -Form, -AICc, -DeltaAICc) %>% 
  ungroup() %>% 
  ...
  knitr::kable(digits = 3, caption = "Number of selected models were variables are present", format.args	= list(big.mark = ','))
```
...
Now we can see the top models that have a delta AICc within 2 of the best model in table  \@ref(tab:BestBacterialPAModels) if expanded
...
<details style=\"margin-bottom:10px;\">
<summary>Model generator bacterial abundance</summary>
```{r ModelGeneratorBacterialPA, cache = T}
METADATAS <- list.files(pattern = "PERMANOVA_BACTERIA_", full.names = T)
...
```{r ModelFittingBacterialPA, cache = T}
...
As seen in table  \@ref(tab:SummaryBacterialPA) there are `r readRDS("FSBacterialPA.rds") %>% 
    dplyr::filter(AICc > -Inf) %>% 
    mutate(DeltaAICc = AICc - min(AICc)) %>% 
    relocate(DeltaAICc, .after = AICc) %>% 
    dplyr::filter(DeltaAICc <= 2) %>% 
...
```{r SummaryBacterialPA, echo = F}
readRDS("FSBacterialPA.rds") %>% 
    dplyr::filter(AICc > -Inf) %>% 
    mutate(DeltaAICc = AICc - min(AICc)) %>% 
    relocate(DeltaAICc, .after = AICc) %>% 
    dplyr::filter(DeltaAICc <= 2) %>% 
...
<details style=\"margin-bottom:10px;\">
<summary>Show table of selected models for bacterial presence</summary>
```{r BestBacterialPAModels, echo=FALSE}
Fs <- readRDS("FSBacterialPA.rds")
Fs %>% 
  dplyr::filter(AICc > -Inf) %>% 
  mutate(DeltaAICc = AICc - min(AICc)) %>% 
  relocate(DeltaAICc, .after = AICc) %>% 
  ...
  select_if(~sum(!is.na(.)) > 0) %>% 
  dplyr::select(-Dataset) %>% 
  knitr::kable(digits = 3, caption = "Best models for bacterial presence absence datasets", format.args	= list(big.mark = ','))
```
</details>
